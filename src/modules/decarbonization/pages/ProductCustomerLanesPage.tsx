...
export default function ProductCustomerLanesPage() {
  ...
  const kpis: KpiMetric[] | undefined = footprint && hotspots && [
    { id: 'co2e', label: 'Annual CO₂e', value: footprint.annualCo2eTonnes, unit: 'tonnes', display: `${formatTonnes(footprint.annualCo2eTonnes)}/yr`, intent: 'neutral', hint: `${footprint.laneCount} lanes` },
    { id: 'cust', label: 'Top customer', value: 0, unit: 'number', display: formatTonnes(hotspots.byCustomer[0]?.co2eTonnes ?? 0), intent: 'risk', hint: hotspots.byCustomer[0]?.label ?? '—' },
    { id: 'prod', label: 'Top product', value: 0, unit: 'number', display: formatTonnes(hotspots.byProductCategory[0]?.co2eTonnes ?? 0), intent: 'risk', hint: hotspots.byProductCategory[0]?.label ?? '—' },
    { id: 'conc', label: 'Top-5 customer share', value: footprint.top5CustomerSharePct, unit: 'percent', intent: footprint.top5CustomerSharePct > 50 ? 'risk' : 'neutral', hint: 'of CO₂e' },
  ];
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'grid', gap: 2.5, gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, mb: 3 }}>
        <ChartContainer title="CO₂e by customer" subtitle="Concentration — bigger tile, bigger footprint">
        ...
        <ChartContainer title="Customer mode mix" subtitle="CO₂e by mode — spot customers with air exposure">
      ...
      <Box sx={{ display: 'grid', gap: 2.5, gridTemplateColumns: { xs: '1fr', lg: '1.4fr 1fr' }, mb: 3 }}>
        <ChartContainer title="Lane priority" subtitle="CO₂e vs intensity · bubble = shipments · top-right = high volume & high intensity (act first)">
          {lanes ? (
            <>
